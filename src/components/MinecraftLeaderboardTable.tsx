            ...
            >
              <div className="flex items-center gap-3 mb-2">
                <div className="flex items-center justify-center">
                  <PlaceIcon place={place} size={28} className="mr-1" />
                </div>
              ...
              <div className="flex items-center justify-center gap-1 pt-2 border-t border-white/10">
                {[
                  ...
                  return (
                    ...
                    >
                      <div className="w-5 h-5 rounded bg-gray-700/70 border border-gray-500/20 flex items-center justify-center mb-0.5">
                        <GameModeIcon mode={mode} className="w-3.5 h-3.5" />
                      </div>
                      <div className={`px-0.5 py-0.5 rounded text-[10px] font-bold ${getTierBadgeColor(tier)} min-w-[18px] text-center`}>
      ...
      <div className="divide-y divide-white/5">
        {players.map((player, index) => {
          ...
          return (
            <div
              key={player.id}
              className="grid grid-cols-12 gap-6 px-6 py-3 cursor-pointer hover:bg-white/5 transform-none items-center"
              onClick={() => handlePlayerRowClick(player)}
            >
              <div className="col-span-1 flex justify-center">
                <PlaceIcon place={place} size={34} />
              </div>
              <div className="col-span-4 flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white/60 text-xs font-medium">({playerPoints})</span>
                  </div>
                </div>
              </div>

              <div className="col-span-2 flex items-center justify-center">
                <RegionBadge region={player.region} size="md" />
              </div>
                  ...
                  {[
                    ...
                    return (
                      <div 
                        ...
                        className="flex flex-col items-center"
                      >
                        <div className="w-6 h-6 rounded-lg bg-gray-700/70 border border-gray-500/20 flex items-center justify-center mb-0.5">
                          <GameModeIcon mode={mode} className="w-4 h-4" />
                        </div>
                        <div className={`px-1 py-0.5 rounded text-[11px] font-bold ${getTierBadgeColor(tier)} min-w-[24px] text-center`}>
                          {tier === 'Not Ranked' ? 'NR' : tier}
                        </div>
                      </div>
                    );
